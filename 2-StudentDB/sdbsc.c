...
int get_student(int fd, int id, student_t *s)
{
	...
	off_t readResult = read(fd, s, STUDENT_RECORD_SIZE);
	if(readResult<0){
		return ERR_DB_FILE;
	}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
	...
	ssize_t readResult = read(fd, &checkStudent, STUDENT_RECORD_SIZE);
	if(readResult<0){
		printf(M_ERR_DB_READ);
		return ERR_DB_FILE;
	}
	if(memcmp(&checkStudent, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE)!=0){
...
int count_db_records(int fd)
{
	...
	int counter = 0;
	while(true){
		ssize_t readResult = read(fd, &student, STUDENT_RECORD_SIZE);
		if(readResult == 0){
			break;
		}
		if(readResult < 0){
